...
from opf_dataset_utils.admittance import extract_branch_admittances
...
from opf_dataset_utils.utils import aggregate_bus_level, get_tap_ratios
...
def calculate_branch_powers(data: HeteroData, predictions: Dict, branch_type: str) -> Tuple[Tensor, Tensor]:
    ...
    T_ij = get_tap_ratios(data, branch_type)

    S_ij = torch.conj(Y_ij + Yc_ij) * (Vm_i / T_ij.abs()) ** 2 - torch.conj(Y_ij * V_j) * V_i / T_ij
